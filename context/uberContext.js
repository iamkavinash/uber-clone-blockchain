...
export const UberProvider = ({children}) => {
    ...
    const checkIfWalletIsConnected = async () => {
      ...
      const requestToCreateUserOnSanity = async address => {
        if (!window.ethereum) return
        try {
          await fetch(`${SERVER_URL}/api/db/createUser`, {
            method: 'POST',
            headers: {
      ...
      useEffect(() => {
        ...
        ;(async () => {
          try {
            const response = await fetch(`${SERVER_URL}/api/map/getDuration`, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
...
const createLocationCoordinatePromise = (locationName,locationType) => {
    ...
    return new Promise(async (resolve,reject) => {
        const response = await fetch(`${SERVER_URL}/api/map/getLocationCoordinates`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
  ...
  const requestToGetCurrentUsersInfo = async walletAddress => {
    try {
      const response = await fetch(
        `${SERVER_URL}/api/db/getUserInfo?walletAddress=${walletAddress}`,
      )
